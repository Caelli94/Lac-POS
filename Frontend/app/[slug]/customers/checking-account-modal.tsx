...
export function CheckingAccountModal({ isOpen, onClose, customer, orgId, account, slug }: Props) {
    if (!customer) return null;

    // customer.credit_balance comes from the table projection, but we want real-time updates from the manager
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <p className="text-slate-500 text-base font-medium flex items-center gap-3">
                            <span className="bg-slate-200/50 px-2 py-0.5 rounded-md font-mono text-slate-600 border border-slate-200/60 text-sm">
                                {customer.doc_number || 'S/DNI'}
                            </span>
                            {customer.email && (
                                <span className="text-slate-400 text-sm">{customer.email}</span>
                            )}
                        </p>
                        <CensoredLimit limit={customer.credit_limit || 0} />
                    </div>
                    ...
                    <ScrollArea className="h-full w-full">
                        <div className="p-6">
                            <CheckingAccountManager
                                customerId={customer.id} // Renamed from clientId
                                orgId={orgId}           // Renamed from organizationId
                                slug={slug}             // Passed slug
