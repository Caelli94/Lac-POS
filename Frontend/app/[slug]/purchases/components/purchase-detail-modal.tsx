...
export function PurchaseDetailModal({ purchase, isOpen, onClose }: Props) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-3xl bg-white rounded-[2rem] p-0 border-none shadow-2xl overflow-hidden h-[80vh] flex flex-col">
                <DialogHeader className="bg-slate-50 p-8 border-b border-slate-100 shrink-0">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                            ...
                            <p className="text-sm font-bold text-slate-700">{new Date(purchase.created_at).toLocaleDateString()}</p>
                        </div>
                        <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                            <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Proveedor</p>
                            <p className="text-sm font-bold text-slate-700 truncate">{purchase.supplier_name || 'Compra Genérica'}</p>
                        </div>
                        <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                            ...
                            <p className="font-bold text-slate-700 flex items-center gap-1.5 leading-tight">
                                <span className="text-sm truncate">{(purchase as any).branches?.name || 'Stock General'}</span>
                            </p>
                        </div>
                        <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                            <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Inversión Total</p>
                            <p className="text-xl font-black text-emerald-600">${purchase.total_amount.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</p>
                        </div>
                    </div>
